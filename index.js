...
function customAssign(...objects) {
  ...
  const union = Object.assign(...objects);
...
function customObjectFromEntries(entries) {
  ...
  const valueEntries = entries.map(([key, value]) => {
  ...
  const newObj = Object.fromEntries(valueEntries);
  return newObj;
...
function getObjectValuesSum(obj) {
  ...
  const value = Object.values(obj);
  const sum = value.reduce((accumulator, value) => accumulator + value, 0);
  return sum;
